...
class Keywords(sw.ScrolledFrame):
        ...
        for d in self._working_keywords:
            ...
            if 'entry' not in widgets:
                ...
                widgets['entry'] = ttk.Entry(
                    ...
                    validate='key',
                    validatecommand=(
                        self.keyword_cb, keyword, row, '%W', '%P', '%s', '%d',
                        '%S'
                    ),
                    takefocus=True,
                    style='Red.TEntry',
    ...
    def handle_keyword(
        self, keyword, row, w_name, value, before, action, changed
    ):
    ...
    def handle_tab(self, event=None, row=None):
        ...
        prefix = lcp(*keywords)
        self.logger.debug(
            'prefix = "{}", current = "{}"'.format(prefix, current)
        )
...
if __name__ == '__main__':  # pragma: no cover
    ...
    metadata = {
        ...
        'ADD-H':
            {
                'description':
                    (
                        'Add hydrogen atoms (intended for use with '
                        'organic compounds)'
                    ),
            },
        'A0': {
        ...
        'AIGOUT':
            {
                'description':
                    (
                        'Print the geometry in Gaussian format in the ARC '
                        'file'
                    ),
            },
        'ALLBONDS':
            {
                'description':
                    (
                        'Print final bond-order matrix, including bonds '
                        'to hydrogen'
                    ),
            },
        'ALLVEC':
        ...
        'ALT_A':
            {
                'description':
                    ('In PDB files with alternative atoms, select '
                     'atoms A'),
                'takes values': 1,
                'default': 'A'
            },
        ...
        'AUX':
            {
                'description':
                    (
                        'Output auxiliary information for use by other '
                        'programs'
                    ),
            },
        'AM1': {
...
#    module_logger.debug('Turned on debugging!')
    ...
    post_button = ttk.Button(
        root, text='Edit Keywords', command=dialog.activate
    )
